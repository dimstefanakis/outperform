      ...
      <div className="flex w-full flex-col bg-black py-10 pt-24 px-4 lg:px-10 rounded-0 md:rounded-[25px]">
        ...
        <div className="flex flex-row">
          <div className="flex flex-col flex-1">
            {data.header && (
              <div className="text-white text-5xl md:text-7xl 5xl:text-[7rem] font-bold mt-6">
                {data.header}
              </div>
            )}
            {data.description && (
              <p className="text-gray-500 hidden md:block text-md md:text-lg 5xl:text-[1.5rem] mt-6 whitespace-break-spaces">
                {data.description}
              </p>
            )}
          </div>
            ...
            <div className="flex flex-col flex-1 items-end md:flex-row justify-end">
              {data.clients.map((client, index) => (
                <ImageBox
                  ...
                  imageClasses="w-[60px] 5xl:w-[100px] md:w-auto h-auto md:h-[30px] mb-10 md:mb-0 md:ml-10 object-contain"
                />
              ))}
            </div>
          )}
